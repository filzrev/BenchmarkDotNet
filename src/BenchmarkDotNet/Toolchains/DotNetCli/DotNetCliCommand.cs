...
namespace BenchmarkDotNet.Toolchains.DotNetCli
{
    public class DotNetCliCommand
    {
        ...
        public async Task<BuildResult> RestoreThenBuildAsync(CancellationToken cancellationToken = default)
        {
            ...
            if (BuildPartition.ForcedNoDependenciesForIntegrationTests)
            {
                var restoreResult = await DotNetCliCommandExecutor.ExecuteAsync(
                    WithArguments(
                        GetRestoreCommand(GenerateResult.ArtifactsPaths, BuildPartition, FilePath, $"{Arguments} --no-dependencies", "restore-no-deps")
                    ),
                ...
                var result = await DotNetCliCommandExecutor.ExecuteAsync(
                    WithArguments(
                        GetBuildCommand(GenerateResult.ArtifactsPaths, BuildPartition, FilePath, TargetFrameworkMoniker, $"{Arguments} --no-restore --no-dependencies", "build-no-restore-no-deps")
                    ),
                    cancellationToken).ConfigureAwait(false);
            ...
            // Autogenerated project use `--no-dependencies` for restore/build
            bool noDependenciesBuild = Path.GetFileName(FilePath) == CsProjGenerator.AutogeneratedProjectName;
            if (noDependenciesBuild)
            {
                var restoreResult = await DotNetCliCommandExecutor.ExecuteAsync(
        ...
        public async Task<BuildResult> RestoreThenBuildThenPublishAsync(CancellationToken cancellationToken = default)
        {
            ...
            var publishResult = await PublishNoBuildAndNoRestore(cancellationToken).ConfigureAwait(false);
        ...
        public Task<DotNetCliCommandResult> PublishNoBuildAndNoRestore(CancellationToken cancellationToken = default)
            => DotNetCliCommandExecutor.ExecuteAsync(
                WithArguments(GetPublishCommand(GenerateResult.ArtifactsPaths, BuildPartition, FilePath, TargetFrameworkMoniker, $"{Arguments} --no-restore --no-build", "publish-no-restore-no-build")),
                cancellationToken);

        public Task<DotNetCliCommandResult> MSBuildNoRestoreAsync(string extraArguments, CancellationToken cancellationToken = default)
        {
            // `dotnet msbuild` command don't restore project by default.
            return DotNetCliCommandExecutor.ExecuteAsync(
                       WithArguments(GetMsBuildCommand(GenerateResult.ArtifactsPaths, BuildPartition, FilePath, TargetFrameworkMoniker, $"{Arguments} {extraArguments}", "msbuild-no-restore")),
                       cancellationToken);
        }
        ...
        internal static string GetRestoreCommand(ArtifactsPaths artifactsPaths, BuildPartition buildPartition, string filePath, string? extraArguments = null, string? binLogSuffix = null)
            => new StringBuilder(256)
                ...
                .MaybeAppendExtraArguments(artifactsPaths, buildPartition, isRestore: true)
                .ToString();
        ...
        internal static string GetBuildCommand(ArtifactsPaths artifactsPaths, BuildPartition buildPartition, string filePath, string tfm, string? extraArguments = null, string? binLogSuffix = null)
            => new StringBuilder(256)
                ...
                .AppendArgument(GetMsBuildBinLogArgument(buildPartition, filePath, binLogSuffix))
                .MaybeAppendExtraArguments(artifactsPaths, buildPartition, isBuild: true)
                .ToString();
        ...
        internal static string GetPublishCommand(ArtifactsPaths artifactsPaths, BuildPartition buildPartition, string filePath, string tfm, string? extraArguments = null, string? binLogSuffix = null)
            => new StringBuilder(256)
                ...
                .AppendArgument(artifactsPaths.PackagesDirectoryName.IsBlank() ? string.Empty : $"/p:NuGetPackageRoot=\"{artifactsPaths.PackagesDirectoryName}\"")
                .AppendArgument(GetMsBuildBinLogArgument(buildPartition, filePath, binLogSuffix))
                .MaybeAppendExtraArguments(artifactsPaths, buildPartition, isPublish: true)
                .ToString();

        internal static string GetMsBuildCommand(ArtifactsPaths artifactsPaths, BuildPartition buildPartition, string filePath, string tfm, string? extraArguments = null, string? binLogSuffix = null)
            => new StringBuilder(256)
                .AppendArgument("msbuild")
                .AppendArgument($"\"{filePath.ToRelativePath(artifactsPaths)}\"")
                .AppendArgument($"/p:TargetFramework={tfm}")
                .AppendArgument($"/p:Configuration={buildPartition.BuildConfiguration}")
                .AppendArgument(GetCustomMsBuildArguments(buildPartition.RepresentativeBenchmarkCase, buildPartition.Resolver))
                .AppendArgument(extraArguments)
                .AppendArgument(GetMandatoryMsBuildSettings(buildPartition.BuildConfiguration))
                .AppendArgument(artifactsPaths.PackagesDirectoryName.IsBlank() ? string.Empty : $"/p:NuGetPackageRoot=\"{artifactsPaths.PackagesDirectoryName}\"")
                .AppendArgument(GetMsBuildBinLogArgument(buildPartition, filePath, binLogSuffix))
                .MaybeAppendExtraArguments(artifactsPaths, buildPartition, isMsBuild: true)
                .ToString();
        ...
        private static string GetMsBuildBinLogArgument(BuildPartition buildPartition, string projectPath, string? suffix)
        {
            ...
            return $"-bl:\"{projectName}-{suffix}.binlog\"";
        }
    ...
    internal static class DotNetCliCommandExtensions
    {
        ...
        internal static StringBuilder MaybeAppendExtraArguments(
            ...
            BuildPartition buildPartition,
            bool isMsBuild = false)
        {
            // Don't use ArtifactsPath for integration tests.
            if (buildPartition.ForcedNoDependenciesForIntegrationTests)
            ...
            if (isMsBuild)
            {
                // `dotnet msbuild` command don't support `--artifacts-path`.
                var artifactsPath = Path.Combine(artifactsPaths.BuildArtifactsDirectoryPath, ".artifacts");
                stringBuilder.AppendArgument($"/p:ArtifactsPath=\"{artifactsPath}\"");

                // Add custom targets file via `CustomAfterMicrosoftCommonTargets` property.
                var customTargetsPath = Path.Combine(artifactsPaths.BuildArtifactsDirectoryPath, "BenchmarkDotNet.Build.targets");
                stringBuilder.AppendArgument($"/p:CustomAfterMicrosoftCommonTargets=\"{customTargetsPath}\"");
            }
            else
            {
