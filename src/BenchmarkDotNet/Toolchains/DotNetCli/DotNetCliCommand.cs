...
namespace BenchmarkDotNet.Toolchains.DotNetCli
{
    public class DotNetCliCommand
    {
        ...
        public Task<DotNetCliCommandResult> PublishNoBuildAndNoRestore(CancellationToken cancellationToken = default)
            => DotNetCliCommandExecutor.ExecuteAsync(
                WithArguments(GetPublishCommand(GenerateResult.ArtifactsPaths, BuildPartition, FilePath, TargetFrameworkMoniker, $"{Arguments} --no-restore --no-build --no-dependencies", "publish-no-restore-no-build")),
                cancellationToken);
        ...
        public Task<DotNetCliCommandResult> MSBuildNoRestoreAsync(string extraArguments, CancellationToken cancellationToken = default)
        {
            return DotNetCliCommandExecutor.ExecuteAsync(
                       WithArguments(GetMsBuildCommand(GenerateResult.ArtifactsPaths, BuildPartition, FilePath, TargetFrameworkMoniker, $"{Arguments} /p:Restore=false {extraArguments}", "msbuild-no-restore")),
                       cancellationToken);
        }
