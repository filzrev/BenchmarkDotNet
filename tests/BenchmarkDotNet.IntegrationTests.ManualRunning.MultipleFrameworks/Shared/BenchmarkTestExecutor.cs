...
public class BenchmarkTestExecutor
{
    ...
    public Reports.Summary CanExecute(Type type, IConfig? config = null, bool fullValidation = true)
    {
        ...
        if (fullValidation)
        {
            ...
            summary.Reports.Should().NotBeEmpty("The \"Summary\" should contain at least one \"BenchmarkReport\" in the \"Reports\" collection");

            summary.CheckPlatformLinkerIssues();

            summary.Reports.Should().AllSatisfy(r => r.BuildResult.IsBuildSuccess.Should().BeTrue());
            summary.Reports.Should().AllSatisfy(r => r.ExecuteResults.Should().NotBeNull());
            summary.Reports.Should().AllSatisfy(r => r.ExecuteResults.Should().AllSatisfy(x => x.IsSuccess.Should().BeTrue()));
        }
